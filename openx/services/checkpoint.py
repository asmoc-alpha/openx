...
class CheckpointManager:
    ...
    def begin_turn(
        ...
        *,
        ...
        作废是有意的语义：用户新起一个回合 = 不再续跑被打断的旧回合，不留
        下会被误恢复的陈旧锚点。**注意这不等于丢掉旧回合的上下文**：被
        打断的那轮已由 ``stream_run`` 截断后并入历史与会话文件（见
        ``OpenXAgent._harvest_interrupted_turn``），旁挂文件只是它的续跑
        锚点，锚点作废不影响历史。``resumed=True`` 时保留--那正是我们刚
        恢复过来的那份。
        """
        if not self.enabled:
            return
