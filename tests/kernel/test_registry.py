...
class TestRegistry:
    def test_register_and_items(self):
        reg = PluginRegistry("tools", validate_tool)
...
class TestValidators:
    ...
    def test_command_ok(self):
        contrib = PluginCommand(_handler, "d", ["al"])
    ...
    def test_command_non_async_handler(self):
        ...
        assert validate_command("c", PluginCommand(sync))

    def test_command_bad_name(self):
        assert validate_command("Bad Name", PluginCommand(_handler))

    def test_command_bad_aliases(self):
        contrib = PluginCommand(_handler, "", ["UPPER"])
